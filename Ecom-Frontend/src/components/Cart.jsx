  ...
  useEffect(() => {
    const fetchImagesAndUpdateCart = async () => {
      ...
      try {
        ...
        const cartItemsWithImages = await Promise.all(
          updatedCartItems.map(async (item) => {
            try {
              ...
              console.warn("Error fetching image for item:", item.id, error);
              return { ...item, imageUrl: item.imageUrl || "placeholder-image-url" };
            }
          })
        );
        console.log("cart",cart)
        ...
        console.warn("Backend down, falling back to local cart items:", error);
        setCartItems(cart);
      }
    };
